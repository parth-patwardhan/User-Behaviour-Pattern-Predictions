...
Then it runs the code that finds the best validation epoch and uses it to calculate the performance of the model.
...
do
    ...
    if [ $type == "$interaction" ]; then
        python evaluate_interaction_prediction.py --network $network --model jodie --epoch ${idx} --cpu
    else
        python evaluate_state_change_prediction.py --network $network --model jodie --epoch ${idx} --cpu
    fi
    (( idx+=1 ))
done 
